  ...
  return (
    <div className={`flex ${isMine ? 'justify-end' : 'justify-start'} group animate-slide-up px-2 sm:px-4 mb-6 sm:mb-8`}>
      <div className={`flex gap-2 sm:gap-3 max-w-[95%] sm:max-w-[85%] lg:max-w-[75%] ${isMine ? 'flex-row-reverse' : 'flex-row'}`}>
        ...
        {/* Avatar */}
        <div className={`w-8 h-8 sm:w-10 sm:h-10 rounded-full flex items-center justify-center flex-shrink-0 mt-2 shadow-lg transition-transform duration-300 group-hover:scale-110 border-2 ${message.senderRole === 'doctor'
        ...
        <div className="flex flex-col space-y-1.5 max-w-full min-w-0">
          ...
          {message.type === 'document-summary' ? (
            !isMine ? (
              ...
              <div className={`rounded-3xl shadow-sm relative transition-all duration-300 px-5 py-4 ${isMine
            ...
            <div className={`rounded-3xl shadow-sm relative transition-all duration-300 group-hover:shadow-md ${isMine
                ? 'bg-gradient-to-br from-indigo-500 to-[#534df2] text-white rounded-br-sm'
                : 'bg-white text-slate-800 border border-slate-100 rounded-bl-sm'
              }`}>
              ...
              <div className={`px-4 sm:px-5 py-3 sm:py-4 break-words ${!isMine && !message.translatedText ? 'pt-4' : ''}`}>
                ...
                {/* Audio Player */}
                {message.audioUrl && (
                  <div className={`mt-4 flex items-center gap-3 p-2 sm:p-3 rounded-2xl transition-all border ${isMine ? 'bg-white/10 border-white/10 hover:bg-white/20' : 'bg-slate-50 border-slate-100 hover:bg-slate-100'
                    }`}>
                    <button
                      onClick={toggleAudio}
                      className={`w-10 h-10 flex items-center justify-center rounded-xl transition-all shadow-sm active:scale-95 ${isMine ? 'bg-white text-indigo-600' : 'bg-indigo-50 text-indigo-600'
                        }`}
                    >
